...
# WHY DID I HAVE THE EXTRA CHANGES?
...
class LightningModule(lightning.LightningModule):
    ...
    def init_metrics_instance(self, num_blocks):
        self.metrics = nn.ModuleList(
            [MeanAveragePrecision(iou_type="segm", backend="faster_coco_eval") for _ in range(num_blocks)]
        )
    ...
    def update_metrics_instance(
        ...
        # print(f'preds = {get_size(preds)  / (1024 ** 2):.1f} MB, targets = {get_size(targets) / (1024 ** 2):.1f} MB.')
