...
#######################################
# Settings used for loading the model #
#######################################
...
for img_idx in [0]:
    ...
    for name in tensor_names:
        ...
        if engine.get_tensor_mode(name) == trt.TensorIOMode.INPUT:
            ...
            if dtype == trt.DataType.UINT8:
                ...
                h_input = np.random.rand(*shape).astype(trt.nptype(dtype))

            host_inputs[name] = h_input
            # Copy to device.
            cuda.memcpy_htod_async(d_buffer, h_input, stream)
        else:
            # Prepare host buffer for output. TODO: I set these to zero.
    ...
    try:
        ...
        t0 = time.time()
        # PyTorch inference, used for comparison.
        sem_pred, inst_pred, sem_target, inst_target = infer_panoptic(img, target)


        t1 = time.time()
        # Compute and save inst. seg. image.
        t3 = plot_panoptic_results(img, sem_pred, inst_pred, sem_target, inst_target, output_path_base)
